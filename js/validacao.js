...
const mensagensDeErro = {
    ...
    senha:{
        ...
        patternMismatch: 'A senha deve conter entre 6 e 12 caracteres'
    },
    dataNascimnto:{
        valueMissing: 'O campo de data não pode estar vazio',
        customError: 'Você deve ser maior de 18 anos para se cadastrar nessa merda'
    },
    cpf:{
    ...
    cep: {
        ...
        customError: 'CEP inválido seu imbecil'
    },
    logradouro: {
...
const validadores = {
    dataNascimnto:input => validaDataNascimento(input),
...
function validaDataNascimento(input) {
    ...
    if(!maiorQue18(dataRecebida)) {
        mensagem = 'Você deve ser maior de 1 anos para se cadastrar nesse site.'
    }
...
function recuperarCEP(input) {
    ...
    if(!input.validity.patternMismatch && !input.validity.valueMissing) {
        fetch(url,options).then(
            ...
            data => {
                if(data.erro) {
                    input.setCustomValidity('CEP inválido seu imbecil')
                    return
                }
